/* 闲然导航 · AI 助手模块  (build: v1.19.0 · 2026-05-09 · 🍵 茶话会模式 · 多代理对话)
...
(function () {
  ...
  const { isGeminiImageModel, formatImageErrorMessage, IMAGE_SIZES, IMAGE_QUALITIES, imageRequest, imageEditRequest } = window.createAIImageAPI({
    buildFetchTarget, buildHttpError, sleepWithSignal, recordProbeOk, recordProbeError,
    recordCooldown, isCooldownError, cooldownSeconds, isRetryableGatewayError,
  });
  ...
  window.AI = {
    ...
    fetchModels,
    chat,
    generateImage: options => window.AITasks.generate(options),
    generateImageEdit: options => window.AITasks.generate(options, true),
    isGeminiImageModel,
