...
> 如需修改账号/密码，请在 `auth.js` 顶部替换 `EXPECTED_HASH` 为你自己的哈希：
>
> ```js
> // 例：在浏览器控制台执行
> await Auth._sha256("新用户名::新密码");
> ```
>
> 换账号后建议同时修改 `TOKEN_SECRET`，使旧设备上已登录的 token 立即失效。
...
```bash
# 打开浏览器控制台，算你的新密码哈希：
await Auth._sha256("你的新用户名::你的新密码")
# 得到 64 位哈希值，替换 auth.js 顶部的 EXPECTED_HASH
# 同时改掉 TOKEN_SECRET 使旧 token 失效
```
