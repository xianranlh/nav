...
- “最近使用”扩展为 20 条并在单卡场景铺满页面；点击后立即提交服务端，刷新不再丢失记录。
- 桌宠新增可见的站岗 / 巡逻切换、三档首页尺寸、位置重置和首页快捷操作，越界位置会自动校正。
- 桌宠配置纳入服务端数据共享，设置页与预览场地升级为深蓝星轨视觉，并保留旧配置自动迁移。
...
- 修复长页面中分组快速导航条被 Flex 压缩、只露出半行按钮的问题，并加固分组头、工具栏和移动端横向溢出。
- 为“本机站点”中的 20 个导航链接加入独立星轨风格图标，覆盖控制台、认证、存储、API、对话、媒体、同步和自动化服务。

### v1.24.0 — 星轨视觉与布局优化

- 全新原创宇宙列车 / 星轨导航品牌图标，统一用于网页标题、登录页、PWA 安装包和桌宠页。
...
- **每个账号数据完全隔离**：导航/设置/AI 会话/提醒/日历/媒体/图库均按账号存储（`user_data` 等表 + `user_id` 列）
- **服务端真鉴权**：登录提交 `SHA-256("用户名::密码")`（明文不过线），服务端签发 session token（7 天 / 勾选保持登录 30 天）；登录接口有限速（每 IP 15 分钟 10 次失败）
- **无缝迁移**：首次启动自动把旧单账号数据归属给用户 #1（admin），凭据沿用原账号；迁移前自动备份 `sakura.db.bak-multiuser`
- **账号管理**：不开放注册；管理员在「设置 → 👥 多账号管理」中添加/删除账号、重置密码（删除账号会连带删除其全部数据与媒体文件）
- **兼容性**：`SAKURA_API_KEY` 仍可用于运维脚本（视为用户 #1）；整库导出/导入（zip）收紧为仅管理员
- 媒体/图库文件读取沿用「随机文件名即密钥」策略（`<img>/<audio>` 无法携带请求头），列表/上传/删除严格按账号隔离
